...
function loadAutomationErrors() { const data = _readJsonFile(automationErrorLogPath(), []); const now = new Date().toISOString(); let changed = false; const entries = (Array.isArray(data) ? data : []).map(e => { const out = { ...e }; if (!out.seen_at) { out.seen_at = out.time || out.timestamp || out.created_at || now; changed = true; } if (out.acknowledged !== true) out.acknowledged = false; return out; }); if (changed) { try { saveAutomationErrors(entries); } catch {} } return entries; }
...
async function _getAutomationConfigs() {
  const r = await _haApiRequest('GET', '/api/config/automation/config', null);
...
async function _automationEntityIdsForConfigIds(configIds) {
  const ids = new Set((configIds || []).map(String).filter(Boolean));
  const out = new Set();
  try {
    const r = await _haApiRequest('GET', '/api/states', null);
    if (r.status >= 200 && r.status < 300) {
      const states = JSON.parse(r.body || '[]');
      if (Array.isArray(states)) states.forEach(st => {
        if (!String(st?.entity_id || '').startsWith('automation.')) return;
        const attrs = st.attributes || {};
        if (ids.has(String(attrs.id || ''))) out.add(st.entity_id);
      });
    }
  } catch(e) { console.warn('[OW-Auto] automation entity lookup failed:', e.message); }
  return [...out];
}
async function setManagedAutomationEnabled(owId, owName, enabled) {
  ...
  const entityIds = await _automationEntityIdsForConfigIds([...generatedIds]);
  let serviceResult = null;
  if (entityIds.length) {
    serviceResult = await _haApiRequest('POST', `/api/services/automation/${draft.enabled ? 'turn_on' : 'turn_off'}`, draft.enabled ? { entity_id:entityIds } : { entity_id:entityIds, stop_actions:false });
  }
  return { ok:true, enabled:draft.enabled, ids:[...generatedIds], entity_ids:entityIds, pushed, deleted, service_status:serviceResult?.status || null, service_detail:serviceResult?.body || null };
}
...
if (pathname === "/ow/alarms/responses/sync" && req.method === "POST") {
  ...
  if (pathname === "/ow/ha-automations" && req.method === "GET") {
    try {
      ...
      // Filter to HA-Overwatch parent automations only. Generated child automations are managed by their parent.
      const ours = allAutomations.filter(a =>
        ((a.alias || "").startsWith("HA-Overwatch") ||
        (a.description || "").includes("ow_meta:") ||
        a.variables?.ow_id) && !_isTurnOffAutomationConfig(a)
      );
  ...
  if (pathname === "/ow/push-automation" && req.method === "POST") {
    try {
      ...
      const cfg  = getHAConfig(loadConfig());
      if (!cfg.ha_url && !process.env.SUPERVISOR_TOKEN) { err(res, "HA not configured", 503); return; }
      ...
      if (process.env.SUPERVISOR_TOKEN) { hostname="supervisor"; port=80; basePath="/core"; token=process.env.SUPERVISOR_TOKEN; lib=http; }
      else { const u = new URL(cfg.ha_url.replace(/\/$/, "")); hostname=u.hostname; port=parseInt(u.port)||(u.protocol==="https:"?443:80); basePath=""; token=cfg.ha_token; lib=u.protocol==="https:"?https:http; }
      function automationConfigRequest(method, id, payload = null) { const body = payload == null ? "" : JSON.stringify(payload); return new Promise((resolve,reject)=>{ const haReq=lib.request({hostname,port,method,path:`${basePath}/api/config/automation/config/${id}`,headers:{"Authorization":`Bearer ${token}`,"Content-Type":"application/json","Content-Length":Buffer.byteLength(body)}},haRes=>{let d="";haRes.on("data",c=>d+=c);haRes.on("end",()=>resolve({status:haRes.statusCode||0,body:d}));}); haReq.on("error",reject); if(body)haReq.write(body); haReq.end(); }); }
      function reloadAutomations(){ return new Promise(resolve=>{ const reloadReq=lib.request({hostname,port,method:"POST",path:`${basePath}/api/services/automation/reload`,headers:{"Authorization":`Bearer ${token}`,"Content-Length":"0"}},r=>{r.resume();resolve();}); reloadReq.on("error",()=>resolve()); reloadReq.end(); }); }
      const pushed=[];
      for (const haAuto of haAutos) { const result=await automationConfigRequest("POST",String(haAuto.id),haAuto); pushed.push({id:haAuto.id,status:result.status,detail:result.body}); if(result.status<200||result.status>=300){json(res,{ok:false,ha_status:result.status,pushed,detail:result.body});return;} }
      const deleted=[];
    const staleIds = new Set([staleTurnOffId].filter(Boolean));
    try {
      const listRes = await _haApiRequest('GET', '/api/config/automation/config', null);
      if (listRes.status >= 200 && listRes.status < 300) {
        ...
        for (const cfgAuto of list) {
          const cid = String(cfgAuto?.id || '');
          const vars = cfgAuto?.variables || {};
          if ((cid === staleTurnOffId || cid.startsWith(`${auto.id}_turn_off_`) || (vars.ow_parent_automation_id === auto.id && vars.ow_child_type === 'turn_off')) && !generatedIds.has(cid)) staleIds.add(cid);
        }
      }
    } catch(e) { console.warn('[OW-Auto] stale turn-off scan failed:', e.message); }
    for (const staleId of [...staleIds]) {
      if (generatedIds.has(staleId)) continue;
      try { const del=await automationConfigRequest("DELETE",staleId,null); deleted.push({id:staleId,status:del.status}); } catch(e) { deleted.push({id:staleId,error:e.message}); }
    }
    await reloadAutomations(); json(res,{ok:true,ha_status:pushed[0]?.status||200,pushed,deleted});
    } catch(e) { err(res,e.message,500); }
    return;
  }
...
/* ── /ow/delete-automation — remove from HA ─────────────────── */
  if (pathname === "/ow/delete-automation" && req.method === "POST") {
    try {
      ...
      function deleteAutomationConfig(deleteId){ return new Promise(resolve=>{ const haReq=lib.request({hostname,port,method:"DELETE",path:`${basePath}/api/config/automation/config/${deleteId}`,headers:{"Authorization":`Bearer ${token}`,"Content-Length":"0"}},haRes=>{let d="";haRes.on("data",c=>d+=c);haRes.on("end",()=>resolve({id:deleteId,status:haRes.statusCode||0,detail:d}));}); haReq.on("error",e=>resolve({id:deleteId,error:e.message})); haReq.end(); }); }
      const idsToDelete = new Set([String(id||''), _automationTurnOffId(id)].filter(Boolean));
      try { const listRes = await _haApiRequest('GET', '/api/config/automation/config', null); if (listRes.status >= 200 && listRes.status < 300) { const parsed = JSON.parse(listRes.body || '[]'); const list = Array.isArray(parsed) ? parsed : (Array.isArray(parsed?.automations) ? parsed.automations : []); for (const cfgAuto of list) { const cid = String(cfgAuto?.id || ''); const vars = cfgAuto?.variables || {}; if (cid.startsWith(`${id}_turn_off_`) || (vars.ow_parent_automation_id === id && vars.ow_child_type === 'turn_off')) idsToDelete.add(cid); } } } catch(e) { console.warn('[OW-Auto] delete child scan failed:', e.message); }
      const deleted=[]; for (const deleteId of [...idsToDelete]) deleted.push(await deleteAutomationConfig(deleteId));
...
function buildAutomationActionBranch(a, actionObj, sourceClearSources = [], auto = null) {
  const seq = [];
  ...
  if (_actionNeedsLifecycleEvent(a, actionObj)) seq.push(_actionLifecycleEventAction(auto, a));
  _pushDynamicTurnOff(seq, a, actionObj, sourceClearSources);
  const triggerFilterCondition = _actionTriggerFilterCondition(a, sourceClearSources);
  const conditions = [...(triggerFilterCondition ? [triggerFilterCondition] : []), ..._automationActionConditions(a)];
...
function pushAutomationAction(actions, a, actionObj, sourceClearSources = []) {
  actions.push(buildAutomationActionBranch(a, actionObj, sourceClearSources, null));
}
...
function _serverEntityKnown(entityId) {
  ...
  try { return (haRegistry.entities || []).some(e => e?.entity_id === id || e?.id === id); } catch { return false; }
}
function _automationHasScopeSelection(a) { return !!((a?.zone_ids || []).length || (a?.group_ids || []).length || (a?.floor_ids || []).length); }
...
function _pruneKnownVisibleEntityIds(entityIds) {
  return [...new Set((entityIds || []).filter(Boolean).map(String))].filter(entityId => _serverEntityKnown(entityId) && !_serverEntityHidden(entityId));
}
function _sanitizeManagedAutomationDraft(auto, zoneList = [], groupList = []) {
  ...
  (clean.actions || []).forEach(a => {
    ['entity_ids','entity_ids_other','entity_ids_extra'].forEach(k => { if (Array.isArray(a[k])) a[k] = _pruneKnownVisibleEntityIds(a[k]); });
...
function _durationToSeconds(value, fallback = 0) {
  const s = String(value || '').trim();
  const m = s.match(/^(\d{1,2}):(\d{1,2}):(\d{1,2})$/);
  if (!m) return fallback;
  return (parseInt(m[1],10)||0)*3600 + (parseInt(m[2],10)||0)*60 + (parseInt(m[3],10)||0);
}
function _secondsToDuration(total) {
  total = Math.max(0, parseInt(total,10)||0);
  const h = Math.floor(total/3600), m = Math.floor((total%3600)/60), s = total%60;
  return [h,m,s].map(v => String(v).padStart(2,'0')).join(':');
}
function _maintainInterval(a) {
  const sec = Math.max(10, _durationToSeconds(a?.maintain_interval || '00:00:20', 20));
  return _secondsToDuration(sec);
}
function _maintainEnabledForAction(a){const clearMode=String(a?.clear_mode||'none');return(a?.maintain_state!==false)&&((a?.maintain_state===true)||(a?.maintain_state===undefined&&clearMode==='source_clears'));}
...
function _actionSupportsMaintain(actionObj) {
  ...
  return ['light','switch','siren'].includes(domain) && ['turn_on','toggle'].includes(service || 'turn_on');
}
function _sourceActiveExpr(sourceClearSources = []) {
  const seen = new Set();
  const parts = [];
  (sourceClearSources || []).forEach(src => {
    const entityId = String(src?.entity_id || '').trim();
    ...
    if (seen.has(key)) return;
    seen.add(key);
    parts.push(`is_state('${_jinjaString(entityId)}', '${_jinjaString(activeState)}')`);
  });
  return parts.length ? parts.join(' or ') : "is_state(trigger.entity_id, 'on')";
}
function _sourceActiveTemplate(sourceClearSources = []) { return `{{ ${_sourceActiveExpr(sourceClearSources)} }}`; }
function _maintainReassertAction(a, actionObj) {
  if (!_maintainEnabledForAction(a) || !_actionSupportsMaintain(actionObj)) return null;
  return _withContinueOnError(JSON.parse(JSON.stringify(actionObj)));
}
function _activeMaintainRepeat(a, actionObj, sourceClearSources = []) {
  const reassert = _maintainReassertAction(a, actionObj);
  if (!reassert || !sourceClearSources.length) return null;
  return { repeat:{ while:[{ condition:'template', value_template:_sourceActiveTemplate(sourceClearSources) }], sequence:[ { delay:_maintainInterval(a) }, { condition:'template', value_template:_sourceActiveTemplate(sourceClearSources) }, reassert ] } };
}
function _cooldownMaintainSequence(a, actionObj, sourceClearSources = []) {
  const reassert = _maintainReassertAction(a, actionObj);
  if (!reassert) return [];
  const clearForSec = _durationToSeconds(a?.clear_for || '00:00:00', 0);
  const intervalSec = Math.max(10, _durationToSeconds(a?.maintain_interval || '00:00:20', 20));
  if (clearForSec <= 0) return [];
  const count = Math.max(1, Math.ceil(clearForSec / intervalSec));
  return [{ repeat:{ count, sequence:[ { condition:'template', value_template:_sourceClearTemplate(sourceClearSources) }, reassert, { delay:_secondsToDuration(Math.min(intervalSec, clearForSec)) } ] } }];
}
...
function _automationTurnOffId(autoOrId) { const id = typeof autoOrId === 'string' ? autoOrId : String(autoOrId?.id || ''); return id ? `${id}_turn_off` : ''; }
function _automationActionTurnOffId(autoOrId, actionOrId) { const autoId = typeof autoOrId === 'string' ? autoOrId : String(autoOrId?.id || ''); const actionId = typeof actionOrId === 'string' ? actionOrId : String(actionOrId?.id || ''); return autoId && actionId ? `${autoId}_turn_off_${actionId}` : ''; }
function _isTurnOffAutomationConfig(a) { const id = String(a?.id || a?.variables?.ow_id || ''); return id.endsWith('_turn_off') || a?.variables?.ow_cleanup === true || a?.variables?.ow_child_type === 'turn_off'; }
function _flattenEntityIds(value) { if (!value) return []; return [...new Set((Array.isArray(value) ? value : [value]).filter(Boolean).map(String))]; }
function _durationObject(value) { const s = _autoDuration(value) || '00:00:00'; const [hours, minutes, seconds] = s.split(':').map(v => Math.max(0, parseInt(v, 10) || 0)); return { hours, minutes, seconds }; }
function _sourceClearSourcesFromTriggers(triggers = []) { const out = [], seen = new Set(); (triggers || []).forEach(t => { if (!t || (t.trigger || t.platform) !== 'state') return; const activeState = String(t.to || 'on'); _flattenEntityIds(t.entity_id).forEach(entityId => { const key = `${entityId}::${activeState}`; if (seen.has(key)) return; seen.add(key); out.push({ entity_id: entityId, active_state: activeState }); }); }); return out; }
function _canUseSimpleStateClearTrigger(sourceClearSources = []) { return sourceClearSources.length === 1 && String(sourceClearSources[0]?.active_state || 'on') === 'on'; }
function _clearTriggerIdForAction(a) { return `clear_${a?.id || 'action'}`; }
function _offTriggerForAction(a, sourceClearSources = [], auto = null) {
  const clearMode = String(a?.clear_mode || 'none');
  if (clearMode !== 'source_clears' && clearMode !== 'conditions') return null;
  return _actionLifecycleEventTrigger(auto, a);
}
function _turnOffChooseBranchForAction(a, actionObj, sourceClearSources = []) {
  const clearMode = String(a?.clear_mode || 'none');
  if (clearMode !== 'source_clears' && clearMode !== 'conditions') return null;
  const offAction = _turnOffActionFor(actionObj);
  ...
  sequence.push({ condition:'template', value_template:_clearTemplateForAction(a, sourceClearSources) });
  sequence.push(_withContinueOnError(offAction));
  return { branch:{ conditions:[{ condition:'trigger', id:triggerId }], sequence } };
}
function _alarmByAutomationId(id) { const s = String(id || ''); return loadAlarms().find(a => String(a.id) === s || String(a.raw_id || '') === s || nameSlug(a.name) === s); }
function _alarmSwitchForAutomationId(id) { const a = _alarmByAutomationId(id); return a ? alarmSwitchEntityIds(a)[0] : `switch.overwatch_alarm_${nameSlug(id)}`; }
function _alarmTriggeredForAutomationId(id) { const a = _alarmByAutomationId(id); const slug = nameSlug(a?.name || id); return [`binary_sensor.overwatch_alarm_${slug}_triggered_armed`, `binary_sensor.overwatch_alarm_${slug}_triggered_disarmed`]; }
function _autoTriggerFilters(t) { return normaliseAlarmTriggerFilters(t?.trigger_filters || t?.filters || null); }
function _autoEnabledFilterTypes(t) { const f = _autoTriggerFilters(t); return ALARM_TRIGGER_FILTER_KEYS.filter(k => f[k]); }
function _autoAllFiltersEnabled(t) { return _autoEnabledFilterTypes(t).length === ALARM_TRIGGER_FILTER_KEYS.length; }
function _autoNoFiltersEnabled(t) { return _autoEnabledFilterTypes(t).length === 0; }
function _zoneIdsForAutoTriggerScope(t, zoneList, groupList, floorList) { const out = new Set(); const add = zid => { if (zid) out.add(String(zid)); }; const zoneIdsForFloor = fid => { const isFirst = floorList.length === 0 || floorList[0]?.id === fid; return zoneList.filter(z => z.floor_id === fid || (!z.floor_id && isFirst)).map(z => z.id); }; (t.floor_ids || []).forEach(fid => zoneIdsForFloor(fid).forEach(add)); (t.group_ids || []).forEach(gid => { const g = groupList.find(x => x.id === gid); (g?.zone_ids || []).forEach(add); }); (t.zone_ids || []).forEach(add); return [...out]; }
...
function _turnOffAutomationForAction(auto, action, actionObj, sourceClearSources) {
  const scopedSources = _actionScopedSourceClearSources(action, sourceClearSources);
  const trig = _offTriggerForAction(action, scopedSources, auto);
...
function buildHAAutomationTurnOffSet(auto, allZones, allGroups, mainAutomation = null) {
  ...
  if (!sourceClearSources.length) return [];
  const zoneList = allZones || [], groupList = allGroups || [], floorList = loadFloors();
  const uniq = ids => [...new Set((ids || []).filter(Boolean))];
  const targetFor = ids => ({ entity_id: uniq(ids) });
  const out = [];
  function addTurnOff(a, actionObj) { const child = _turnOffAutomationForAction(auto, a, actionObj, sourceClearSources); if (child) out.push(child); }
  for (const a of (auto.actions || [])) {
    if (a.type === 'siren') { const ids = uniq([..._resolveAutomationScopedEntityIds(a,'sirens',zoneList,groupList,floorList), ..._pruneZoneStoredEntityIds(a,'sirens',zoneList,groupList,floorList), ...(a.entity_ids||[]), ...(a.entity_ids_extra||[])]); if (ids.length) addTurnOff(a,{action:`siren.${a.service||'turn_on'}`,target:targetFor(ids)}); }
    else if (a.type === 'light') { const ids = uniq([..._resolveAutomationScopedEntityIds(a,'lights',zoneList,groupList,floorList), ..._pruneZoneStoredEntityIds(a,'lights',zoneList,groupList,floorList), ...(a.entity_ids_other||[]), ...(a.entity_ids||[])]); if (ids.length) addTurnOff(a,{action:`light.${a.service||'turn_on'}`,target:targetFor(ids)}); }
    else if (a.type === 'camera_view') { const ids = uniq([..._resolveAutomationScopedEntityIds(a,'cameras',zoneList,groupList,floorList), ..._pruneZoneStoredEntityIds(a,'cameras',zoneList,groupList,floorList), ..._pruneKnownVisibleEntityIds(a.entity_ids)]); if (ids.length) addTurnOff(a,{action:`switch.${a.service||'turn_on'}`,target:targetFor(ids)}); }
    else if (a.type === 'entity' && a.entity_id) { const domain = a.entity_id.split('.')[0]; addTurnOff(a,{action:`${domain}.${a.service||'turn_on'}`,target:{entity_id:[a.entity_id]}}); }
    else if (a.type === 'arm') { const ids = uniq(a.entity_ids||[]); if (ids.length) addTurnOff(a,{action:`switch.${a.service||'turn_on'}`,target:targetFor(ids)}); }
  }
  return out;
}
function buildHAAutomationTurnOff(auto, allZones, allGroups, mainAutomation = null) { return buildHAAutomationTurnOffSet(auto, allZones, allGroups, mainAutomation)[0] || null; }

function buildHAAutomationSet(auto, allZones, allGroups) { const cleanAuto = _sanitizeManagedAutomationDraft(auto, allZones || [], allGroups || []); const main = buildHAAutomation(cleanAuto, allZones, allGroups); return [main, ...buildHAAutomationTurnOffSet(cleanAuto, allZones, allGroups, main)]; }
...
function buildHAAutomation(auto, allZones, allGroups) {
  ...
  for (const t of (auto.triggers || [])) {
    ...
    if (t.type === 'zone' || t.type === 'zone_arm') {
      ...
      const toState = isArm ? (t.state === 'armed' ? 'on' : 'off') : (t.event === 'triggered' ? 'on' : 'off');
      let entityIds = [];
      if (!isArm && _autoNoFiltersEnabled(t)) { triggers.push({ trigger:"template", value_template:"{{ false }}", id:`disabled_${t.id || uid_simple()}` }); continue; }
      const useAggregateZoneEntities = isArm || _autoAllFiltersEnabled(t);
      if (useAggregateZoneEntities) {
        (t.floor_ids || []).forEach(fid => { if (isArm) entityIds.push(`switch.overwatch_zone_floor_${fid}`); else _zoneIdsForFloor(fid).forEach(zid => entityIds.push(`binary_sensor.overwatch_zone_${zoneSlugById(zid)}_triggered`)); });
        (t.group_ids || []).forEach(gid => { const slug = groupSlugById(gid); entityIds.push(isArm ? `switch.overwatch_zone_group_${slug}` : `binary_sensor.overwatch_zone_group_${slug}_triggered`); });
        (t.zone_ids || []).forEach(zid => { const slug = zoneSlugById(zid); entityIds.push(isArm ? `switch.overwatch_zone_${slug}` : `binary_sensor.overwatch_zone_${slug}_triggered`); });
      } else entityIds = _filteredZoneSensorEntityIdsForTrigger(t, zoneList, groupList, floorList);
      entityIds = _uniqList(entityIds);
      if (entityIds.length > 0) { _addSourceClearSources(entityIds, toState); const trig = { trigger:"state", entity_id:entityIds, to:toState }; if (forDur) trig.for = forDur; triggers.push(trig); }
      else { const fallback = isArm ? "switch.overwatch_zone_master" : "binary_sensor.overwatch_zone_master_triggered"; if (isArm || _autoAllFiltersEnabled(t)) { _addSourceClearSources(fallback, toState); triggers.push({ trigger:"state", entity_id:[fallback], to:toState }); } else triggers.push({ trigger:"template", value_template:"{{ false }}", id:`no_matching_filtered_sources_${t.id || uid_simple()}` }); }

    } else if (t.type === 'alarm') {
      let entityIds = [];
      const event = String(t.event || 'triggered');
      const alarmIds = t.alarm_ids || [];
      let toState = 'on';
      if (event === 'enabled' || event === 'disabled') {
        entityIds = alarmIds.map(_alarmSwitchForAutomationId);
        toState = event === 'enabled' ? 'on' : 'off';
      } else {
        entityIds = alarmIds.flatMap(_alarmTriggeredForAutomationId);
        toState = event === 'triggered' ? 'on' : 'off';
      }
      entityIds = _uniqList(entityIds);
      if (entityIds.length) {
  ...
  for (const c of (auto.conditions || [])) {
    ...
    } else if (c.type === 'zone_arm') {
      const state = c.state === 'disarmed' ? 'off' : 'on';
      _automationZoneArmEntities(c, zoneList, groupList, floorList).forEach(entityId => conditions.push({ condition:'state', entity_id:entityId, state }));
  ...
  function addActionBranch(a, actionObj) {
    actionBranches.push(buildAutomationActionBranch(a, actionObj, sourceClearSources, auto));
  }
